...
int inside_MicroPay(int Errtimes)
{
	...
	if(ret) 
	{
		...
		{
			APP_ShowTradeMsg("ÌáÐÑ!ÓÉÓÚÍøÂçÎÊÌâ\nÎ´ÊÕµ½½»Ò×½á¹û\nÇë´ÓÊÖ»ú»òÖÕ¶Ë²éÑ¯¹¦ÄÜ\nÈ·¶¨½á¹û!",5000);
		}
		return ret;
	}
RE_QUERY_ADDER:
...
int OrderQuery(char* pTitle)
{
	...
	do{
		...
		if(ret==OPER_HARD)
		{
		    if(APP_ScanInput(pTitle,"ÇëÊÖ¶¯ÊäÈëÉÌ»§µ¥ºÅ",NULL,g_ColData.transactionId,8,32)) 
				break;
		}
		APP_ShowWaitFor(NULL);//STR_NET_LINK_WLAN
		g_ColData.tradeId[0]=0;
...
int shiftStatV2(char* pTitle)
{
	...
	do{
		...
		if(pCode[0] == '0') 
		{
			...
			{
				*(--pshow)='\0';	//×îºóÒ»ÐÐ²»ÓÃ»»ÐÐ
				if(EVENT_OK == APP_ShowInfoMsg(pTitle,showbuff,"°´[È·ÈÏ]¼ü½»°à",30*1000))
				{
